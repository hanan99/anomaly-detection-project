...
# # Load the video file
# video_path = "data/VIDEO TO ELABORATE_2.mp4"
...
# output_path = "output/anomaly_detection_output.mp4"
...
#     # Apply thresholding to enhance anomalies
#     _, thresh = cv2.threshold(fgmask, 25, 255, cv2.THRESH_BINARY)

#     # Optical Flow Analysis (if previous frame exists)
...
#         # Highlight abnormal movements
#         mask = np.zeros_like(frame) # Create an empty mask same size as the frame
#         mask[..., 1] = 255  # Set color to green for flow visualization
#         mask[..., 0] = (ang * 180 / np.pi / 2).astype(np.uint8) # Hue (Direction)
#         mask[..., 2] = cv2.normalize(mag, None, 0, 255, cv2.NORM_MINMAX).astype(np.uint8) # Brightness (Speed)
        
#         # Convert to BGR and overlay
